...
public final class Main {
    ...
    private void process() {
        ...
        while (_input.hasNextLine()) {
            ...
            if (!(nextLine.contains("*"))) {
                printMessageLine(newEnigma.convert(nextLine));
            } else {
                setUp(newEnigma, nextLine);
            }
        }
    }
    ...
    private Machine readConfig() {
        try {
            ...
            return new Machine(_alphabet, numSlots, pawls, allRotors);
    ...
    private Rotor readRotor() {
        try {
            ...
            String notch = typeOfRotor.substring(1);
            StringBuilder cyc = new StringBuilder(_config.nextLine());
            while (_config.hasNext("\\(.*")) {
                cyc.append(_config.nextLine());
            }
            Permutation perm = new Permutation(cyc.toString(), _alphabet);

            if (typeOfRotor.charAt(0) == 'N') {
                newRotor = new FixedRotor(name, perm);
            } else if (typeOfRotor.charAt(0) == 'M') {
                newRotor = new MovingRotor(name, perm, notch);
            } else {
    ...
    private void setUp(Machine M, String settings) {
        int numRotor = M.numRotors();
        String[] rotorNames = new String[numRotor];
        ...
        if (scan.hasNext("[*]")) {
            ...
            while (scan.hasNext("[(]\\w+[)]")) {
                plugboard.append(scan.next()).append(" ");
            }
            if (plugboard.length() > 0) {
                M.setPlugboard(new Permutation(plugboard.toString(),
                        _alphabet));
            }
